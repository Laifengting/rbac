    ...
    <body>
                ...
                <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <button type="button" class="btn btn-danger" onclick="batchDeleteUsers()" style="float:right;margin-left:10px;">
                                <i
        ...
        <script type="text/javascript">
                    ...
                    success: function (response) {
                        ...
                        if (response.success) {
                            ...
                            $.each(userList, function (index, user) {
                                ...
                                tableContent += '        <button type="button" class="btn btn-primary btn-xs" onclick="goUpdatePage(' + user.id + ')"><i class=" glyphicon glyphicon-pencil"></i></button>';
                                tableContent += '        <button type="button" class="btn btn-danger btn-xs" onclick="deleteUser(' + user.id + ',\'' + user.loginacct + '\')"><i class=" glyphicon glyphicon-remove"></i></button>';
                                tableContent += '    </td>';
                                tableContent += '</tr>';
                            });
            ...
            function deleteUser(id, loginacct) {
                var loadingIndex;
                layer.confirm("删除用户信息【" + loginacct + "】,是否继续", {icon: 3, title: '提示'},
                    function (cindex) {
                        // 确认回调，删除用户信息
                        $.ajax({
                            type: "POST",
            ...
            function batchDeleteUsers() {
                ...
                if (boxes.length === 0) {
                    layer.msg("至少要选中一个需要删除的用户", {time: 2000, icon: 5, shift: 6}, function () {
                    });
                } else {
                    var loadingIndex;
                    layer.confirm("删除选中的所有用户信息,是否继续", {icon: 3, title: '提示'},
                        function (cindex) {
                            // 确认回调，删除选择的用户信息
                            $.ajax({
                                type: "POST",
                                url: "${APP_PATH}/user/batchDelete",
                                // 将表单内容序列化为字符串。
            ...
            function goAssignPage(userId) {
                window.location.href = "${APP_PATH}/user/assign?id=" + userId;
            }
        </script>
    </body>
</html>
